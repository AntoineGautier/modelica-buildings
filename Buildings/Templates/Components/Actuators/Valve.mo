...
model Valve
  ...
  extends Buildings.Fluid.Interfaces.PartialTwoPortInterface(
    final m_flow_nominal=dat.m_flow_nominal) annotation(__ctrlFlow(
    enable=false));
  ...
  final parameter Boolean is_twoWay = typ ==
    ...
    annotation(Evaluate=true);
  final parameter Boolean is_thrWay = typ ==
  ...
  parameter Buildings.Templates.Components.Types.ValveCharacteristicTwoWay chaTwo =
    if is_actMod
    then Buildings.Templates.Components.Types.ValveCharacteristicTwoWay.EqualPercentage
  ...
  parameter Buildings.Templates.Components.Types.ValveCharacteristicThreeWay chaThr =
    if is_actMod
    then Buildings.Templates.Components.Types.ValveCharacteristicThreeWay.EqualPercentageLinear
  ...
  parameter Buildings.Templates.Components.Data.Valve dat(final typ=typ)
    ...
    annotation(__ctrlFlow(enable=false),
      Dialog(enable=typ <> Buildings.Templates.Components.Types.Valve.None),
    ...
    annotation(__ctrlFlow(enable=false),
      Dialog(tab="Dynamics",
        ...
        enable=use_strokeTime
          and typ <> Buildings.Templates.Components.Types.Valve.None));
    ...
    annotation(Evaluate=true,
      Dialog(tab="Advanced",
        enable=typ <> Buildings.Templates.Components.Types.Valve.None
          and (if is_twoWay
            then chaTwo <>
              Buildings.Templates.Components.Types.ValveCharacteristicTwoWay.PressureIndependent
            else true)));
  ...
  Modelica.Fluid.Interfaces.FluidPort_a portByp_a(
    ...
    if typ == Buildings.Templates.Components.Types.Valve.ThreeWayTwoPosition
      or typ == Buildings.Templates.Components.Types.Valve.ThreeWayModulating
  ...
  Buildings.Controls.OBC.CDL.Conversions.BooleanToReal y1(
    ...
    annotation(Placement(transformation(extent={{-10,-10},{10,10}},
      rotation=-90,
      origin={-80,120})));
  Modelica.Blocks.Routing.RealPassThrough y
    if is_actMod
    "Modulating signal"
    annotation(Placement(transformation(extent={{-10,-10},{10,10}},
      rotation=-90,
      origin={-40,120})));
  Modelica.Blocks.Routing.RealPassThrough y_actual
    if is_actMod
    "Position feedback"
    annotation(Placement(transformation(extent={{10,-10},{-10,10}},
      rotation=-90,
      origin={0,120})));
  Buildings.Controls.OBC.CDL.Reals.GreaterThreshold y1_actual(h=0.5E-2, t=0.99)
    if is_actTwo
    "Open end switch status"
    annotation(Placement(transformation(extent={{10,-10},{-10,10}},
      rotation=-90,
      origin={80,120})));
  Buildings.Controls.OBC.CDL.Reals.LessThreshold y0_actual(h=0.5E-2, t=0.01)
    if is_actTwo
  ...
  Buildings.Fluid.Actuators.Valves.TwoWayEqualPercentage equ(
    ...
    if is_twoWay
      and chaTwo ==
        Buildings.Templates.Components.Types.ValveCharacteristicTwoWay.EqualPercentage
...
equation
  ...
  connect(port_a, non.port_a)
    annotation(Line(points={{-100,0},{-74,0},{-74,40},{-10,40}},
      color={0,127,255}));
  connect(non.port_b, port_b)
    annotation(Line(points={{10,40},{80,40},{80,0},{100,0}},
      color={0,127,255}));
  connect(port_a, equ.port_a)
    annotation(Line(points={{-100,0},{-50,0}},
      color={0,127,255}));
  connect(equ.port_b, port_b)
    annotation(Line(points={{-30,0},{100,0}},
      color={0,127,255}));
  connect(lin.port_b, port_b)
    annotation(Line(points={{-50,20},{80,20},{80,0},{100,0}},
      color={0,127,255}));
  connect(port_a, lin.port_a)
    annotation(Line(points={{-100,0},{-74,0},{-74,20},{-70,20}},
      color={0,127,255}));
  connect(port_a, ind.port_a)
    annotation(Line(points={{-100,0},{-74,0},{-74,-20},{-30,-20}},
      color={0,127,255}));
  connect(port_a, tab.port_a)
    annotation(Line(points={{-100,0},{-74,0},{-74,-40},{-10,-40}},
      color={0,127,255}));
  connect(ind.port_b, port_b)
    annotation(Line(points={{-10,-20},{80,-20},{80,0},{100,0}},
      color={0,127,255}));
  connect(tab.port_b, port_b)
    annotation(Line(points={{10,-40},{80,-40},{80,0},{100,0}},
      color={0,127,255}));
  connect(bus.y1, y1.u)
    annotation(Line(points={{0,160},{0,140},{-80,140},{-80,132}},
      color={255,204,51},
      thickness=0.5));
  connect(bus.y, y.u)
    annotation(Line(points={{0,160},{0,140},{-40,140},{-40,132}},
      color={255,204,51},
      thickness=0.5));
  connect(bus.y_actual, y_actual.y)
    annotation(Line(points={{0,160},{0,131}},
      color={255,204,51},
      thickness=0.5));
  connect(equLin.port_3, portByp_a)
    annotation(Line(points={{40,-90},{40,-120},{0,-120},{0,-160}},
      color={0,127,255}));
  connect(port_a, equLin.port_1)
    annotation(Line(points={{-100,0},{-74,0},{-74,-80},{30,-80}},
      color={0,127,255}));
  connect(equLin.port_2, port_b)
    annotation(Line(points={{50,-80},{80,-80},{80,0},{100,0}},
      color={0,127,255}));
  connect(bus.y0_actual, y0_actual.y)
    annotation(Line(points={{0,160},{0,140},{40,140},{40,132}},
      color={255,204,51},
      thickness=0.5));
  connect(bus.y1_actual, y1_actual.y)
    annotation(Line(points={{0,160},{0,140},{80,140},{80,132}},
      color={255,204,51},
      thickness=0.5));
  connect(tabTab.port_2, port_b)
    annotation(Line(points={{70,-100},{80,-100},{80,0},{100,0}},
      color={0,127,255}));
  connect(tabTab.port_3, portByp_a)
    annotation(Line(points={{60,-110},{60,-120},{0,-120},{0,-160}},
      color={0,127,255}));
  connect(port_a, tabTab.port_1)
    annotation(Line(points={{-100,0},{-74,0},{-74,-100},{50,-100}},
      color={0,127,255}));
  connect(port_a, linLin.port_1)
    annotation(Line(points={{-100,0},{-74,0},{-74,-60},{10,-60}},
      color={0,127,255}));
  connect(linLin.port_3, portByp_a)
    annotation(Line(points={{20,-70},{20,-120},{0,-120},{0,-160}},
      color={0,127,255}));
  connect(linLin.port_2, port_b)
    annotation(Line(points={{30,-60},{80,-60},{80,0},{100,0}},
      color={0,127,255}));
annotation(Icon(graphics={Line(points={{-100,0},{100,0}},
  color={0,0,0},
  thickness=5),
Line(visible=is_thrWay,
  points={{0,-100},{0,0}},
  color={0,0,0},
  thickness=5),
Bitmap(visible=is_actMod,
...
<p>
  ...
  with a variety of characteristics. The supported types of valve are
  described in the enumeration
  <a href=\"modelica://Buildings.Templates.Components.Types.Valve\">
    Buildings.Templates.Components.Types.Valve</a>. The supported flow
  characteristics are described in the enumeration
  <a href=\"modelica://Buildings.Templates.Components.Types.ValveCharacteristicTwoWay\">
    Buildings.Templates.Components.Types.ValveCharacteristicTwoWay</a> for
  two-way valves, and in the enumeration
  <a href=\"modelica://Buildings.Templates.Components.Types.ValveCharacteristicThreeWay\">
    Buildings.Templates.Components.Types.ValveCharacteristicThreeWay</a> for
  three-way valves.
</p>
<h4>Control points</h4>
<p>The following input and output points are available.</p>
<p>For modulating valves:</p>
<ul>
  <li>
    The valve position is modulated with a fractional position signal
    <code>y</code> (real).<br />
    <code>y = 0</code> corresponds to fully closed (fully open bypass for
    three-way valves). <code>y = 1</code> corresponds to fully open (fully
    closed bypass for three-way valves).
  </li>
  <li>
...
<p>For two-position valves:</p>
<ul>
  <li>
    The valve position is commanded with a Boolean signal <code>y1</code>.<br />
    <code>y1 = false</code> corresponds to fully closed (fully open bypass for
    three-way valves). <code>y1 = true</code> corresponds to fully open (fully
    closed bypass for three-way valves).
  </li>
  <li>
    The open end switch status <code>y1_actual</code> and closed end switch
    status <code>y0_actual</code> (Booleans) are returned.<br />
    <code>y1_actual = false</code> corresponds to fully closed.
    <code>y1_actual = true</code> corresponds to fully open. And the opposite
    for <code>y0_actual</code>.
  </li>
</ul>
...
<p>
  The default characteristic is equal percentage (resp. equal percentage and
  linear) for modulating two-way valves (resp. modulating three-way valves).
  The default characteristic is linear for two-position actuators.
</p>
<p>
  For three-way valves, the default setting for the ratio of the
  <i>Kvs</i> coefficient between the bypass branch and the direct branch is
  <code>fraK=1.0</code>, see
  <a href=\"modelica://Buildings.Fluid.HydronicConfigurations.UsersGuide.ControlValves\">
    Buildings.Fluid.HydronicConfigurations.UsersGuide.ControlValves</a> for
  the rationale.
</p>
</html>",
    revisions="<html>
<ul>
  <li>
    ...
    Added leakage parameter, refactored using proxy parameters in enable annotations.
  </li>
  <li>
