...
equation
  ...
  connect(dpHeaWatRem[1].port_a, loaHea.port_a)
    annotation(Line(points={{60,-108},{60,-100},{90,-100}},
      color={0,127,255}));
  connect(ratLoa.y[2], loaCoo.u)
    annotation(Line(points={{-158,40},{80,40},{80,-32},{88,-32}},
      color={0,0,127}));
  connect(ratLoa.y[1], loaHea.u)
    annotation(Line(points={{-158,40},{80,40},{80,-92},{88,-92}},
      color={0,0,127}));
  connect(loaHea.yVal_actual, reqPlaRes.uHeaCoiSet)
    annotation(Line(points={{112,-92},{130,-92},{130,44},{92,44}},
      color={0,0,127}));
  connect(enaLoa.y, loaCoo.u1)
    annotation(Line(points={{-158,0},{76,0},{76,-36},{88,-36}},
      color={255,0,255}));
  connect(enaLoa.y, loaHea.u1)
    annotation(Line(points={{-158,0},{76,0},{76,-96},{88,-96}},
      color={255,0,255}));
  ...
  connect(volHeaWat.ports[2], loaHea.port_a)
    annotation(Line(points={{1,-100},{90,-100}},
      color={0,127,255}));
  connect(pipHeaWat.port_b, pla.port_aHeaWat)
    annotation(Line(points={{30,-140},{-30,-140},{-30,-98},{-40,-98}},
      color={0,127,255}));
  ...
  connect(volChiWat.ports[2], loaCoo.port_a)
    annotation(Line(points={{1,-40},{90,-40}},
      color={0,127,255}));
...
<p>
  ...
  <a href=\"modelica://Buildings.Templates.Plants.HeatPumps.AirToWater\">
    Buildings.Templates.Plants.HeatPumps.AirToWater</a> by simulating a
  <i>24</i>-hour period with overlapping heating and cooling loads. The
...
<p>
  ...
  between a cooling and heating system (the default setting) and a
  heating-only system. Advanced equipment and control options can be modified
  via the parameter dialog of the plant component.
</p>
<p>Simulating this model shows how the plant responds to a varying load by</p>
<ul>
  <li>
    staging or unstaging the AWHPs and the HRC with the associated primary
    pumps,
  </li>
  <li>rotating lead/lag alternate equipment to ensure even wear,</li>
  <li>
...
<ul>
  ...
  <li>
    ...
    Added sidestream HRC and refactored the model after updating the HP plant
