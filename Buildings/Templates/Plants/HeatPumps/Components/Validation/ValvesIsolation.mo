...
model ValvesIsolation
  ...
  parameter Data.Controller datCtlHeaInl(
    cfg(
      ...
      have_valPhpOutIso=valIsoHeaInl.have_valPhpOutIso,
      have_valPhpInlIso=valIsoHeaInl.have_valPhpInlIso,
      have_chiWat=false,
      typPumHeaWatPri=Buildings.Templates.Plants.HeatPumps.Types.PumpsPrimary.Variable,
      typPumChiWatPri=Buildings.Templates.Plants.HeatPumps.Types.PumpsPrimary.None,
      typTanHeaWat=Buildings.Templates.Components.Types.IntegrationPoint.None,
    ...
    staHp={fill(1, datCtlHeaInl.cfg.nHp)}) "Controller parameters"
  ...
  Fluid.FixedResistances.PressureDrop hpSep[valIsoSep.nHp](
    redeclare each final package Medium=Medium,
    m_flow_nominal=valIsoSep.mHeaWatUni_flow_nominal,
  ...
  Buildings.Templates.Plants.HeatPumps.Interfaces.Bus busPla
    ...
    annotation(Placement(iconVisible=false,
      transformation(extent={{80,-140},{120,-100}}),
  ...
  Buildings.Templates.Components.Interfaces.Bus busPumHeaWatPri
    ...
    annotation(Placement(iconVisible=false,
      transformation(extent={{100,-346},{140,-306}}),
  ...
  Buildings.Templates.Components.Interfaces.Bus busPumChiWatPri
    ...
    annotation(Placement(iconVisible=false,
      transformation(extent={{120,-366},{160,-326}}),
      iconTransformation(extent={{-466,50},{-426,90}})));
...
equation
  ...
  connect(retChiWat.ports[1], valIsoCom.port_aChiWat)
    annotation(Line(points={{260,310},{240,310}},
      color={0,127,255}));
  connect(valIsoCom.port_bChiWat, supChiWat.ports[1])
    annotation(Line(points={{240,350},{260,350}},
      color={0,127,255}));
  connect(valIsoCom.port_bHeaWat, supHeaWat.ports[1])
    annotation(Line(points={{-240,290},{-260,290}},
      color={0,127,255}));
  ...
  connect(ctlSep.bus, valIsoSep.bus)
    annotation(Line(points={{-30,-120},{0,-120},{0,-170}},
      color={255,204,51},
      thickness=0.5));
  ...
  connect(busPumHeaWatPri.y1, yPumHeaWatPri.u)
    annotation(Line(points={{120,-326},{82,-326}},
      color={255,204,51},
      thickness=0.5));
  connect(yPumHeaWatPri.y, pumHeaWatPri.y)
    annotation(Line(points={{58,-326},{0,-326},{0,-320},{-54,-320}},
      color={0,0,127}));
  ...
  connect(yPumChiWatPri.y, pumChiWatPri.y)
    annotation(Line(points={{88,-346},{0,-346},{0,-334},{-22,-334}},
      color={0,0,127}));
  ...
  connect(hpSep.port_b, pumChiWatPri.port_a)
    annotation(Line(points={{-10,-360},{-34,-360},{-34,-344}},
      color={0,127,255}));
  connect(hpSep.port_b, pumHeaWatPri.port_a)
    annotation(Line(points={{-10,-360},{-66,-360},{-66,-330}},
      color={0,127,255}));
...
<ul>
  <li>
    ...
    and isolation valves at both heat pump inlet and outlet: component
    <code>valIsoCom</code>.
  </li>
  <li>
    Heating-only system with isolation valves at heat pump inlet: component
    <code>valIsoHeaInl</code>.
  </li>
  <li>
    Heating and cooling system with separate dedicated primary HW and CHW
    pumps and isolation valves at heat pump inlet: component
    <code>valIsoSep</code>.
  </li>
</ul>
<p>
