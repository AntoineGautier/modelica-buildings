...
partial model PartialChilledWaterLoop
  ...
  Buildings.Templates.Components.Sensors.VolumeFlowRate VChiWatByp_flow(
    ...
    final have_sen=(typDisChiWat ==
      ...
      and ctl.typMeaCtlChiWatPri ==
        Buildings.Templates.Plants.Chillers.Types.PrimaryOverflowMeasurement.FlowDecoupler,
    final typ=Buildings.Templates.Components.Types.SensorVolumeFlowRate.FlowMeter,
    icon_pipe=Buildings.Templates.Components.Types.IntegrationPoint.Supply)
  ...
  Buildings.Templates.Components.Sensors.DifferentialPressure dpChiWatLoc(
    ...
    if not ctl.have_senDpChiWatRemWir
  ...
  replaceable Buildings.Templates.Plants.Chillers.Components.Economizers.None eco(
    final show_T=show_T)
    constrainedby Buildings.Templates.Plants.Chillers.Components.Interfaces.PartialEconomizer(
      redeclare final package MediumChiWat=MediumChiWat,
  ...
  replaceable Buildings.Templates.Plants.Chillers.Components.Interfaces.PartialController ctl
    constrainedby Buildings.Templates.Plants.Chillers.Components.Interfaces.PartialController(
...
equation
  ...
  connect(inlPumChiWatSec.ports_b, pumChiWatSec.ports_a)
    annotation(Line(points={{200,0},{200,0}},
      color={0,127,255}));
  connect(pumChiWatSec.ports_b, outPumChiWatSec.ports_a)
    annotation(Line(points={{220,0},{220,0}},
      color={0,127,255}));
  connect(intChi.ports_bRet[1:nChi], chi.ports_aChiWat)
    annotation(Line(points={{40,-260},{40,-192}},
      color={0,127,255},
      visible=viewDiagramAll));
  connect(chi.ports_bChiWat, intChi.ports_aSup[1:nChi])
    annotation(Line(points={{40,0},{40,0}},
      color={0,0,0},
      thickness=0.5));
  connect(intChi.ports_bRet[nChi + 1], eco.port_a)
    annotation(Line(
      points={{40,-260},{40,-236},{-64,-236},{-64,-280},{-10,-280},{-10,-252}},
      color={0,127,255},
      visible=viewDiagramAll));
  connect(eco.port_b, intChi.ports_aSup[nChi + 1])
    annotation(Line(points={{10,-252},{40,-252},{40,0}},
      color={0,127,255},
      visible=viewDiagramAll));
  connect(inlConChi.ports_b, chi.ports_aCon)
    annotation(Line(points={{-50,-192},{-40,-192}},
      color={0,127,255},
      visible=viewDiagramAll));
  connect(inlConChi.port_aComLeg, eco.port_aConWat)
    annotation(Line(points={{-60,-192},{-60,-244},{-40,-244}},
      color={0,0,0},
      thickness=0.5,
      visible=typEco <>
        Buildings.Templates.Plants.Chillers.Types.Economizer.None));
  connect(eco.port_bConWat, outConChi.ports_a[nChi + 1])
    annotation(Line(points={{-40,-260},{-40,0},{-40,0}},
      color={0,0,0},
      thickness=0.5,
  ...
  connect(dpChiWatLoc.port_a, port_b)
    annotation(Line(points={{280,-110},{280,0},{300,0}},
      color={0,127,255},
      visible=viewDiagramAll));
  connect(dpChiWatLoc.port_b, port_a)
    annotation(Line(points={{280,-130},{280,-260},{300,-260}},
      color={0,127,255},
      visible=viewDiagramAll));
  connect(outPumChiWatPri.port_b, VChiWatPri_flow.port_a)
    annotation(Line(points={{120,0},{120,0}},
      color={0,0,0},
      thickness=0.5));
  ...
  connect(junByp.port_3, valChiWatMinByp.port_a)
    annotation(Line(points={{170,-10},{170,-90}},
      color={0,0,0},
      thickness=0.5));
  connect(junByp.port_2, supChiWat.port_a)
    annotation(Line(points={{180,0},{200,0}},
      color={0,0,0},
      thickness=0.5));
  connect(outPumChiWatSec.port_b, VChiWatSecSup_flow.port_a)
    annotation(Line(points={{240,0},{250,0}},
      color={0,127,255}));
  connect(supChiWat.port_b, VChiWatSecSup_flow.port_a)
    annotation(Line(points={{220,0},{250,0}},
      color={0,0,0},
      thickness=0.5));
  connect(VChiWatSecSup_flow.port_b, port_b)
    annotation(Line(points={{270,0},{300,0}},
      color={0,0,0},
      thickness=0.5));
  connect(port_a, VChiWatSecRet_flow.port_a)
    annotation(Line(points={{300,-260},{250,-260}},
      color={0,0,0},
      thickness=0.5,
      pattern=LinePattern.Dash));
  connect(VChiWatSecRet_flow.port_b, TChiWatSecRet.port_a)
    annotation(Line(points={{230,-260},{210,-260}},
      color={0,0,0},
      thickness=0.5,
      pattern=LinePattern.Dash));
  connect(TChiWatSecRet.port_b, intChi.port_aRet)
    annotation(Line(points={{190,-260},{80,-260}},
      color={0,0,0},
      thickness=0.5,
      pattern=LinePattern.Dash));
  connect(VChiWatPri_flow.port_b, TChiWatPriSup.port_a)
    annotation(Line(points={{140,0},{140,0}},
      color={0,0,0},
      thickness=0.5));
  connect(TChiWatPriSup.port_b, junByp.port_1)
    annotation(Line(points={{160,0},{160,0}},
      color={0,0,0},
      thickness=0.5));
  connect(VChiWatByp_flow.port_b, intChi.port_aByp)
    annotation(Line(points={{170,-160},{170,-230},{40,-230}},
      color={0,0,0},
      thickness=0.5));
  connect(valChiWatMinByp.port_b, VChiWatByp_flow.port_a)
    annotation(Line(points={{170,-110},{170,-140}},
      color={0,0,0},
      thickness=0.5));
  connect(busWea, out.weaBus)
    annotation(Line(points={{0,280},{0,250},{0.2,250}},
      color={255,204,51},
      thickness=0.5));
  connect(out.ports[1], TOut.port)
    annotation(Line(points={{-1,230},{40,230}},
      color={0,127,255}));
  connect(phiOut.port, out.ports[2])
    annotation(Line(points={{-40,230},{1,230}},
      color={0,127,255}));
  connect(bus, ctl.bus)
    annotation(Line(points={{-300,200},{-10,200}},
      color={255,204,51},
      thickness=0.5));
  connect(ctl.busAirHan, busAirHan)
    annotation(Line(points={{10,206},{280,206},{280,240},{300,240}},
      color={255,204,51},
      thickness=0.5));
  connect(ctl.busEquZon, busEquZon)
    annotation(Line(points={{10,194},{280,194},{280,160},{300,160}},
      color={255,204,51},
      thickness=0.5));
...
annotation(Documentation(
  info="<html>
<p>
  This serves as the base class to build chilled water plant templates. The
  model boundaries are the condenser inlet and outlet ports on the CW side,
  and the supply and return ports on the CHW side. The following components
  are included.
</p>
<ul>
  <li>
  ...
  <li>
    ...
    then the secondary CHW pumps are not included in this model and must be
    modeled separately from the plant.
  </li>
  <li>
...
<ul>
  ...
  Diagram(graphics={Line(points={{-40,0},{-60,0}},
    color={0,0,0},
    pattern=LinePattern.Dash,
    thickness=0.5,
