...
model ChillersToPrimaryPumps
  ...
  parameter Buildings.Templates.Plants.Chillers.Types.Distribution typDisChiWat
    ...
    annotation(Evaluate=true,
      Dialog(group="Configuration"));
  parameter Integer nPumChiWatPri(start=1, final min=1) = nChi
    "Number of primary CHW pumps"
    annotation(Evaluate=true,
      Dialog(group="Configuration",
        enable=typArrPumChiWatPri ==
  ...
  Modelica.Fluid.Interfaces.FluidPorts_b ports_bRet[nPorts](
    ...
    annotation(Placement(transformation(extent={{-210,-140},{-190,-60}}),
      iconTransformation(extent={{-210,-1340},{-190,-1260}})));
  Modelica.Fluid.Interfaces.FluidPorts_b ports_bSup[nPumChiWatPri](
    ...
    annotation(Placement(transformation(extent={{190,80},{210,160}}),
      iconTransformation(extent={{190,1260},{210,1340}})));
  Modelica.Fluid.Interfaces.FluidPort_a port_aByp(
    ...
    "CHW supply from minimum flow bypass or common leg"
    annotation(Placement(transformation(extent={{190,-10},{210,10}}),
      iconTransformation(extent={{-210,-1010},{-190,-990}})));
  ...
  Buildings.Templates.Components.Routing.MultipleToMultiple rouSupPar(
    ...
    final tau=tau,
    final have_comLeg=typArrPumChiWatPri ==
      Buildings.Templates.Components.Types.PumpArrangement.Headered,
    icon_dy=0)
    if typArrChi ==
      Buildings.Templates.Plants.Chillers.Types.ChillerArrangement.Parallel
  ...
  Buildings.Templates.Components.Routing.PassThroughFluid rouSupRetSer[nChi -
    1](redeclare final package Medium=MediumChiWat)
    if nChi > 1
      and typArrChi ==
        Buildings.Templates.Plants.Chillers.Types.ChillerArrangement.Series
  ...
  Buildings.Templates.Components.Sensors.VolumeFlowRate VChiWatPri_flow(
    ...
    final m_flow_nominal=mChiWatPri_flow_nominal,
    final allowFlowReversal=allowFlowReversal,
    final have_sen=have_senVChiWatPri
...
equation
  ...
  connect(rouSupPar.ports_b, ports_bSup)
    annotation(Line(points={{10,120},{200,120}},
      color={0,127,255}));
  connect(valChiWatChiBypPar.port_b, rouSupPar.port_aComLeg)
    annotation(Line(points={{1.77636e-15,10},{0,10},{0,120}},
      color={0,127,255}));
  connect(TChiWatEcoBef.port_b, ports_bRet[nChi + 1])
    annotation(Line(points={{-150,-120},{-200,-120},{-200,-100}},
      color={0,127,255}));
  connect(rouSupSer.ports_b, ports_bSup)
    annotation(Line(points={{10,140},{140,140},{140,120},{200,120}},
      color={0,127,255}));
  connect(ports_aSup[nChi + 1], TChiWatEcoAft.port_a)
    annotation(Line(points={{-200,120},{-200,100},{140,100},{140,70}},
      color={0,127,255}));
  connect(port_aRet, rouRet.port_a)
    annotation(Line(points={{200,-100},{180,-100},{180,-120},{160,-120}},
      color={0,127,255}));
  connect(rouRet.port_b, TChiWatEcoBef.port_a)
    annotation(Line(points={{140,-120},{-130,-120}},
      color={0,127,255}));
  connect(TChiWatPlaRet.port_b, rouRetChiPar.port_a)
    annotation(Line(points={{70,-100},{10,-100}},
      color={0,127,255}));
  connect(TChiWatPlaRet.port_b, rouRetChiSer.port_a)
    annotation(Line(points={{70,-100},{20,-100},{20,-72},{10,-72}},
      color={0,127,255}));
  connect(rouRetChiSer.port_b, ports_bRet[nChi])
    annotation(Line(points={{-10,-72},{-20,-72},{-20,-100},{-200,-100}},
      color={0,127,255}));
  connect(TChiWatPlaRet.port_b, valChiWatChiBypPar.port_a)
    annotation(Line(points={{70,-100},{40,-100},{40,-40},{0,-40},{0,-10}},
      color={0,127,255}));
  connect(rouSupRetSer.port_b, ports_bRet[1:nChi - 1])
    annotation(Line(points={{-120,-10},{-120,-100},{-200,-100}},
      color={0,127,255}));
  connect(ports_aSup[1:nChi], junSupChiSer.port_1)
    annotation(Line(points={{-200,120},{-200,140},{-170,140}},
      color={0,127,255}));
  connect(junSupChiSer[1].port_2, rouSupSer.port_a)
    annotation(Line(points={{-150,140},{-10,140}},
      color={0,127,255}));
  connect(junSupChiSer[2:nChi].port_2, rouSupRetSer.port_a)
    annotation(Line(points={{-150,140},{-120,140},{-120,10}},
      color={0,127,255}));
  connect(port_aByp, junByp.port_3)
    annotation(Line(points={{200,0},{150,0}},
      color={0,127,255}));
  connect(TChiWatEcoAft.port_b, junByp.port_1)
    annotation(Line(points={{140,50},{140,10}},
      color={0,127,255}));
  connect(rouRetChiPar.ports_b, ports_bRet[1:nChi])
    annotation(Line(points={{-10,-100},{-200,-100}},
      color={0,127,255}));
  connect(ports_aSup[1:nChi], rouSupPar.ports_a)
    annotation(Line(points={{-200,120},{-10,120}},
      color={0,127,255}));
  connect(junByp.port_2, VChiWatPri_flow.port_a)
    annotation(Line(points={{140,-10},{140,-100},{130,-100}},
      color={0,127,255}));
  connect(VChiWatPri_flow.port_b, TChiWatPlaRet.port_a)
    annotation(Line(points={{110,-100},{90,-100}},
      color={0,127,255}));
  connect(com.port_a, rouSupPar.port_aComLeg)
    annotation(Line(points={{0,160},{0,120}},
      color={0,127,255}));
annotation(defaultComponentName="int",
  Icon(coordinateSystem(preserveAspectRatio=false,
    extent={{-200,-1340},{200,1340}}),
    graphics={Line(points={{200,-1300},{-200,-1300}},
      color={0,0,0},
      thickness=5,
      pattern=LinePattern.Dash),
    Text(extent={{-149,-1350},{151,-1390}},
      textColor={0,0,255},
      textString="%name"),
    ...
    Line(visible=typArrChi ==
      ...
      color={0,0,0},
      thickness=5),
    Line(visible=typArrPumChiWatPri ==
      Buildings.Templates.Components.Types.PumpArrangement.Headered,
    ...
    Line(visible=have_valChiWatChiBypPar,
      ...
      color={0,0,0},
      thickness=5),
    Bitmap(visible=have_valChiWatChiBypPar,
    ...
    Bitmap(extent={{-100,-100},{100,100}},
      ...
      origin={120,-1240},
      rotation=360),
    Bitmap(extent={{80,-1140},{160,-1060}},
      visible=typEco <>
        Buildings.Templates.Plants.Chillers.Types.Economizer.None,
      fileName="modelica://Buildings/Resources/Images/Templates/Components/Sensors/Temperature.svg"),
    Bitmap(extent={{-100,-100},{100,100}},
  ...
  Documentation(
    info="<html>
<p>This model represents the hydronic interface between</p>
<ul>
  <li>
    the CHW supply and return connections of the plant components (chillers
    and optional waterside economizer),
  </li>
  <li>
    the inlet manifold of the primary CHW pumps (where the pumps are assumed
    to be on the CHW supply side),
  </li>
  <li>the CHW return pipe, and</li>
  <li>
    the optional bypass (common leg for primary-secondary systems and bypass
