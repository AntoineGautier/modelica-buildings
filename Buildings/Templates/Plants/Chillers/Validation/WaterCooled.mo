...
equation
  connect(mulInt[1].y, busAirHan.reqResChiWat)
    annotation(Line(points={{-12,60},{-40,60}},
      color={255,127,0}));
  ...
  connect(TAirSup.y, reqPlaRes.TAirSup)
    annotation(Line(points={{-158,80},{128,80},{128,68},{122,68}},
      color={0,0,127}));
  connect(TAirSup.y, reqPlaRes.TAirSupSet)
    annotation(Line(points={{-158,80},{128,80},{128,63},{122,63}},
      color={0,0,127}));
  ...
  connect(reqPlaRes.yChiWatResReq, mulInt[1].u2)
    annotation(Line(points={{98,68},{80,68},{80,54},{12,54}},
      color={255,127,0}));
  connect(reqPlaRes.yChiPlaReq, mulInt[2].u2)
    annotation(Line(points={{98,63},{80,63},{80,54},{12,54}},
      color={255,127,0}));
  connect(schEna.y, busPla.u1SchEna)
    annotation(Line(points={{-158,0},{-80,0},{-80,-20}},
      color={255,0,255}));
  connect(loa.port_b, mChiWat_flow.port_a)
    annotation(Line(points={{120,-40},{160,-40},{160,-70}},
      color={0,127,255}));
  connect(ratLoa.y[1], loa.u)
    annotation(Line(points={{-158,40},{80,40},{80,-32},{98,-32}},
      color={0,0,127}));
  connect(loa.yVal_actual, reqPlaRes.uCooCoiSet)
    annotation(Line(points={{122,-32},{140,-32},{140,57},{122,57}},
      color={0,0,127}));
  connect(dpChiWatRem[1].port_a, loa.port_a)
    annotation(Line(points={{60,-70},{60,-40},{100,-40}},
      color={0,127,255}));
  connect(schEna.y, loa.u1)
    annotation(Line(points={{-158,0},{76,0},{76,-36},{98,-36}},
      color={255,0,255}));
  connect(pla.port_b, vol.ports[1])
    annotation(Line(points={{-39.8,-80},{-20,-80},{-20,-40},{-1,-40}},
      color={0,127,255}));
  connect(vol.ports[2], loa.port_a)
    annotation(Line(points={{1,-40},{100,-40}},
      color={0,127,255}));
  connect(pipChiWat.port_b, pla.port_a)
    annotation(Line(points={{20,-120},{-20,-120},{-20,-90},{-39.8,-90}},
      color={0,127,255}));
annotation(__Dymola_Commands(
  file="modelica://Buildings/Resources/Scripts/Dymola/Templates/Plants/Chillers/Validation/WaterCooled.mos"
    "Simulate and plot"),
...
<p>
  ...
  <a href=\"modelica://Buildings.Templates.Plants.Chillers.WaterCooled\">
    Buildings.Templates.Plants.Chillers.WaterCooled</a> by simulating a
...
<ul>
  <li>staging or unstaging the chillers,</li>
  <li>
    resetting the supply temperature and remote differential pressure in the
    CHW loop based on the valve position,
  </li>
  <li>
