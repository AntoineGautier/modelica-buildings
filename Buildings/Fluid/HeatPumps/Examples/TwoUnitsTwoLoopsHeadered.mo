...
model TwoUnitsTwoLoopsHeadered
...
equation
  ...
  connect(booToRea.y, P_2.y)
    annotation (Line(points={{-228,180},{20,180},{20,152}},color={0,0,127}));
  ...
  connect(booToRea1.y, P_1.y) annotation (Line(points={{-228,-160},{20,-160},{20,
          -188}},                  color={0,0,127}));
  ...
  connect(TE_4.port_b, V_3.port_a) annotation (Line(points={{-110,60},{-120,60},
      ...
      thickness=1));
  connect(jun4.port_2, P_2.port_a) annotation (Line(points={{-180,90},{-180,140},
          {10,140}}, color={0,127,255}));
  connect(P_2.port_b, BUFFER_HHW.ports[2])
    annotation (Line(points={{30,140},{159,140}}, color={0,127,255}));
  connect(jun2.port_2, TE_3.port_a)
    annotation (Line(points={{70,60},{-10,60}}, color={0,127,255}));
  connect(jun.port_2, P_1.port_a) annotation (Line(points={{-200,-110},{-200,-200},
          {10,-200}}, color={0,127,255}));
  connect(P_1.port_b, decChiWat.port_a) annotation (Line(points={{30,-200},{200,
          -200},{200,-180}}, color={0,127,255}));
  connect(EXP.ports[1], TE_1.port_a) annotation (Line(points={{50,-102},{52,-102},
          {52,-80},{-10,-80}}, color={0,127,255}));
  connect(jun1.port_2, TE_1.port_a)
...
__Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/HeatPumps/Examples/TwoUnitsTwoLoopsHeadered.mos"
        "Simulate and plot"),
    Documentation(info="<html>
<p>
This model reproduces the standard configuration from TRANE (2022),
modified with primary headered pumps instead of dedicated pumps as in 
Buildings.Fluid.HeatPumps.Examples.TwoUnitsTwoLoops.
